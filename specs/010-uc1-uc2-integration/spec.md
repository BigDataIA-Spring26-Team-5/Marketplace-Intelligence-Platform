...
**Feature Branch**: `010-uc1-uc2-integration`  
**Created**: 2026-04-21  
**Status**: Draft  
**Owner**: Aqeel  
...
- **FR-001**: UC1 pipeline runner MUST call `emit_event()` at block_start and block_end for every block in both Stage A (per-source preprocessing) and Stage B (cross-source dedup). Stage B blocks emit both block_start/block_end AND dedup_cluster events.
- **FR-002**: UC1 pipeline runner MUST call `MetricsCollector().push()` once per source immediately after Stage A completes for that source, preserving per-source label granularity in Prometheus (`source`, `run_id` grouping labels).
- **FR-003**: UC1 MUST emit `run_started` at the top of the run and `run_completed` in a `finally` block so it fires even on failure.
- **FR-004**: Quarantine logic MUST call `emit_event()` for each rejected row with a `reason` string; `row_data` MUST contain only `product_name`, `brand_name`, `ingredients`, and the specific field that triggered rejection — NOT the full row.
- **FR-005**: fuzzy_dedup / collapse_cluster MUST emit `dedup_cluster` events for every cluster produced.
- **FR-006**: All `emit_event()` calls MUST be fire-and-forget (spawned thread or non-blocking producer) and wrapped in try/except — pipeline MUST NOT block on Kafka acknowledgement and MUST NOT crash due to UC2 emission failures.
- **FR-007**: `run_id` MUST be consistent across all events for the same run (pass it as a parameter from the top-level runner).
- **FR-008**: `null_rates` in `block_end` events MUST be computed over columns defined in `NULL_RATE_COLUMNS` constant in `src/pipeline/runner.py`; default value is `["product_name", "brand_name", "ingredients", "primary_category"]`.
...
# null_rates keys are defined by NULL_RATE_COLUMNS constant in src/pipeline/runner.py
# Default: ["product_name", "brand_name", "ingredients", "primary_category"]

# Quarantine
{"event_type": "quarantine", "run_id": str, "source": str,
 "row_hash": str,
 "row_data": {"product_name": str, "brand_name": str, "ingredients": str, "<offending_field>": any},
 "reason": str, "ts": iso_str}
# row_data is NOT the full row — only product_name, brand_name, ingredients, plus the specific field that triggered rejection
...
src/agents/graph.py             ← emit quarantine events (quarantine logic lives here)
```
...
- `metrics_collector.py` and `kafka_to_pg.py` exist in `src/uc2_observability/` on the shared branch and will be available after pull.
- Quarantine emission goes in `src/agents/graph.py` (not `src/ui/hitl3.py`) since quarantine logic lives there in the current codebase.
- S3 in the metrics dict maps to `embedding` (FAISS KNN) in the current enrichment stats; S4 maps to `llm` (LLM tier). S3 cluster propagation referenced in the spec applies to Stage B cross-source dedup, not the current single-source enrichment.
...
- Q: Should `emit_event()` calls block the pipeline (sync) or be fire-and-forget (async)? → A: Fire-and-forget — spawn thread or use non-blocking producer to avoid adding Kafka ACK latency to pipeline wall time.
- Q: How many times per pipeline invocation should `MetricsCollector().push()` be called? → A: Once per source after Stage A completes (not once per full run), to preserve per-source label granularity in Prometheus.
- Q: Which columns should `null_rates` cover in `block_end` events? → A: Configurable list via `NULL_RATE_COLUMNS` constant in `src/pipeline/runner.py` (default: `product_name`, `brand_name`, `ingredients`, `primary_category`).
- Q: Should `row_data` in quarantine events contain the full row or a trimmed subset? → A: Key fields only — `product_name`, `brand_name`, `ingredients`, plus the specific field that triggered the schema contract failure.
- Q: Should block_start/block_end events cover Stage B blocks (dedup) in addition to Stage A? → A: All blocks — Stage A and Stage B both emit block_start/block_end; Stage B additionally emits dedup_cluster events.

---

## Files NOT to modify

- `src/uc2_observability/metrics_collector.py` — already implemented.
- `src/uc2_observability/kafka_to_pg.py` — already implemented.
- `config/prometheus.yml` — already configured.
- Any Docker or GCP infrastructure config.
